...
- **ThroughNet Phase A / A1 — onboarding flow + signature paper-tear intro (`tn-onboarding`).** Designed and built the first-run setup UX (the IA gap ADR-151 §7.A3 only sketched). A **cover page** shows the ThroughNet wordmark in an elegant calligraphy script (**Allura**, per a user reference) on an aged-parchment ground (procedural: warm base + light centre + vignette + coarse-stain & fine-mottle fractal noise — no stock image), and gently **sways like hung paper**. Hitting *Begin* runs the signature transition: the sheet **rips down a jagged, interlocking seam** straight through the title, the two halves **buckle, ball up tight, and fling off** (a 2-stage GSAP crumple with crease shading) to reveal a **six-step wizard** behind it — *prepare (doctor preflight) → flash → connect → place → calibrate → done* — built from the shared chrome kit, each step grounded in the real mechanics (serial/`ufw` checks, `espflash`, `provision.py --auto` fleet auto-id with a live fleet list, the bistatic placement diagram + per-link RSSI, empty-room baseline). `tn-app` now routes first-run → onboarding and `finish` → the live diorama home (`?view=live` jumps straight there). Iterated visually via a standalone prototype (`app/prototypes/onboarding.html`) + a multi-frame screenshot harness, then ported to a `tn-onboarding` Lit component (`src/styles/onboarding.css` scoped to the element). Home model confirmed: **diorama-as-home**, data as overlay cards. Builds clean (33 modules). Step panels are mock; the real localhost `/api/v1/setup/*` endpoints land in A3.
