...
### Added
- **ThroughNet Phase A / fleet management — OTA-over-network firmware updates (the last fleet-mgmt unit).** Deployed nodes can now be updated from the Devices console without USB. The ESP32 firmware already implemented a PSK-authed A/B OTA endpoint (`POST /ota` on port 8032, `Authorization: Bearer <psk>`, fail-closed per ADR-050, rollback on bad boot) — this wires the **host side**. (1) **`provision.py --ota-psk <hex>`** writes the key to the firmware's `security` NVS namespace (declared after `csi_cfg` so it partitions correctly; round-trips in per-port state as a `MERGEABLE_ATTR`, so a later partial provision never wipes it; the whole NVS partition is regenerated each flash). (2) **The sensing-server sets the key automatically** — `setup_provision` generates a fresh 256-bit key (`/dev/urandom`, sha2 fallback) and passes `--ota-psk` whenever a board has none on record, so a single USB (re-)provision makes an existing node network-updatable; the key is never re-generated once set. (3) **`POST /api/v1/setup/ota {node}`** resolves the node's *own* LAN IP from the source address of its live CSI (DHCP-dynamic — now tracked per node as `NodeState::last_src_ip`, learned from `recv_from` on both the CSI and edge-vitals ingest paths), reads its OTA key from the per-port state, and streams the bundled app image (offset `0x20000`, embedded or on-disk via the same resolver as flash) to `<ip>:8032/ota` with the Bearer key (blocking `ureq` off-thread; honest errors incl. 403 = stale-key → re-provision). (4) The reconciler exposes per-device **`has_ota_psk`** + **`ota_ready`** (online ∧ key on record ∧ IP known); the Devices console replaces the "update · soon" stub with a real **armed OTA button** on *deployed* (not-on-USB) nodes — confirm → push → the node A/B-flashes & reboots (~20 s, rollback). A **beacon-only TX has no learnable IP** (sends no CSI), so it's honestly USB-only; offline / un-keyed nodes show the disabled button with the reason. 2 new tests (PSK gen = 64 hex + unique; hex encode) → 153 sensing-server tests pass; `provision.py` CSV builder + PSK validation checked; app builds; the full arm→confirm→success flow + the per-state button reasons verified in `?mock`; the `/api/v1/setup/ota` route + the new `has_ota_psk`/`ota_ready` fields confirmed live on the running binary. **Verified end-to-end on hardware (2026-06-15)** — the OTA push to node 1 flipped `ota_0`→`ota_1`, and the whole fleet was subsequently flashed + keyed (see the fleet-wide rollout entry under *Fixed*). ADR-151 §7 (fleet mgmt) + ADR-050. **This completes the fleet-management track** (reconciler → state-aware onboarding → Devices console → Server/Ops → OTA).
- **ThroughNet Phase A / fleet management — the Server/Ops page (`tn-server`).** The "more dashboard control" ask as its own **"server"** tab: runtime **status** (version, source, uptime, nodes online, WS clients, pid), a **live tail of the server's own log**, and **restart** / **shut down** controls (each behind an inline confirm). The log is a bounded in-memory ring (cap 800 lines) fed by a tee'd `tracing` fmt writer (ANSI off, also still to stdout), with monotonic per-line indices for incremental polling (2 s). New endpoints **`GET /api/v1/server/status`**, **`GET /api/v1/server/logs?since=N`**, **`POST /api/v1/server/restart`** (re-exec self with the same args — works standalone and under systemd), **`POST /api/v1/server/shutdown`** (clean `exit(0)`); all under `/api/v1/*` so they inherit the optional bearer gate + loopback-default bind (same posture as flash/provision — the page notes "controls are localhost-only"). **Verified live on a throwaway instance:** status + incremental logs read; restart re-exec'd (two startup banners, ports rebound); shutdown exited clean (exit 0, port freed, and the shutdown `WARN` itself shows up in the captured ring). 1 new test (ring split/index/cap/clamp) → 11/11 setup tests; app builds; page verified in `?mock`. ADR-151 §7 (fleet mgmt). Remaining: OTA-over-network firmware updates for deployed nodes.
- **ThroughNet Phase A / fleet management — the Devices console (`tn-devices`).** A new **"devices"** tab: the standalone fleet console the product was missing — see every board the system knows (provisioned ∪ streaming) from the reconciler and control each one *whenever you want*, not just during first-run. The live view polls the **fast** reconciler (state ∪ live) every 3 s; USB probing is **on-demand** ("scan USB") only, never on the poll (esptool resets boards). Expand a board to **re-provision** (wi-fi / password / role) or **re-flash**: role is a dropdown (auto / **TX · illuminator** / RX · receiver) so the illuminator can be assigned/replaced on purpose — the previously-missing "flash an illuminator separately" flow. Re-flashing a provisioned board is **armed** (two-step confirm) and the TX shows the ~20 s mesh-blackout warning; a board that isn't on USB says "nothing to do — plug in to act on it"; OTA update is stubbed ("soon", disabled) pending the OTA unit. Server: `setup_provision` now accepts an explicit **`role`** (`tx`|`rx`, validated — `legacy` rejected) and forwards `--role` to `provision.py`, overriding `--auto`'s first-board-is-TX; `provisionBoard()` gains `role`/`targetIp`. 1 new test (role guard) → 10/10 setup tests; app builds; console verified in `?mock` (device list, present/offline branching, the action form + role select). ADR-151 §7 (fleet mgmt). Next: OTA-over-network + a Server/Ops page.
- **ThroughNet Phase A / fleet management (foundation) — state-aware onboarding that stops demanding re-flashes.** Fixes the reported footgun: re-running setup always told the user to re-flash every board (the wizard had no concept of "already set up"), and re-flashing the TX illuminator collapsed the mesh (every radio-silent RX is locked to the TX's MAC/channel, so when the TX rebooted they all went dark — "only 1 node detected"). New server-side reconciler **`GET /api/v1/setup/fleet`** joins the per-port provision-state files (what's configured) with live `node_states` (what's actually streaming) into one role-aware view; each board is bucketed `streaming` (fresh frames → working, skip it) / `provisioned` (known but silent → offline/placement/TX-reboot, *not* a re-flash) / `unprovisioned` (only seen on USB → needs first-time setup), and `summary.healthy` is true once ≥1 TX + ≥1 RX stream. `?scan=true` also probes USB for `present`/chip and surfaces brand-new boards (default is the fast state+live join). Credentials never leave the box — only `has_password` is exposed. Onboarding (`tn-onboarding` + `tn-setup.ts`) now consumes it: a healthy fleet **short-circuits** the wizard to an "already set up" screen (node list + *enter*), the flash step recognizes an already-provisioned/streaming board and offers **continue** with "re-flash anyway" demoted to a deliberate, guarded secondary, and a **TX guard** warns before re-flashing the illuminator (the ~20 s mesh blackout). Refactors `sanitize_port`/`provision_state_dir` (mirror `provision.py`) and a shared `PortProbe`/`probe_serial_ports` out of `setup_scan` (no behavior change). 3 new unit tests (bucket classifier, port-sanitize round-trip) → 9/9 setup tests; reconciler verified against the live node-3 RX state file (join + credential redaction + buckets); both onboarding screens verified in `?mock`. Foundation for the Devices console + OTA. ADR-151 §7 (fleet mgmt).
- **ThroughNet Phase A / A4 — always-on deployment artifacts (systemd + docker-compose).** A focused `deploy/` for running the bundled single binary headless, separate from the inherited multi-service `docker/` stack (the wrong, heavy RuView setup): `deploy/throughnet.service` (systemd unit — `--source esp32`, restart-on-failure, after network-online, hardened, with install notes incl. the `--features bundle` build + the `ufw` open), `deploy/Dockerfile` (multi-stage: builds the `bundle` binary, ships it on `debian:bookworm-slim`, no Python — targeted COPYs so `v2/target` never enters the context and the embed `#[folder]` paths resolve), and `deploy/docker-compose.yml` (`network_mode: host` so multi-node ESP32 UDP + mDNS work; `CSI_SOURCE`/`RUVIEW_API_TOKEN` knobs). Made `--source` honor the **`CSI_SOURCE` env var** (CLI still wins) so the compose/systemd knob actually controls the source — **verified**: `CSI_SOURCE=simulated ./sensing-server` → `/health` reports `source: simulated`. systemd unit structurally validated (`systemd-analyze verify`); `docker compose config` valid. The container image build itself wraps the proven native `--features bundle` build and is a release/CI step (not built in-sandbox). Self-documenting via each artifact's header comments; the full docs rewrite is Phase 4.4. ADR-151 §7 (A4).
- **ThroughNet Phase A / A4 (start) — embed the ESP32 firmware into the binary (self-contained flash).** The shipped artifact now carries its own firmware: a new **`embed-firmware`** cargo feature `rust-embed`s `firmware/esp32-csi-node/release_bins/throughnet-s3-nodisp` (the 4 S3 bins) into the binary, and a **`bundle`** feature = `embed-ui` + `embed-firmware` builds the full self-contained product binary (UI + firmware). The flash endpoint resolves firmware via a new `resolve_firmware_dir()`: prefer an on-disk dir (dev + `THROUGHNET_FIRMWARE_DIR` override), else extract the embedded bins to a temp dir (`tempfile`, kept alive across the flash) — so flashing no longer needs a `release_bins/` directory at runtime. `scripts/throughnet-app.sh` now builds `--features bundle`. **Verified the gold-standard way (as A2 was):** built the release `bundle` binary, **moved both `app/dist` and the firmware dir away**, and confirmed the binary still serves the embedded UI (`<title>ThroughNet</title>` at `/`), scans the board, **and flashes a real ESP32-S3 from the embedded firmware** (hash verified, 17 s) — then restored the dirs. 2 new unit tests (resolver prefers on-disk; errors when absent + not embedded); default/`embed-firmware` both compile; suite green. Remaining A4: systemd + docker-compose for always-on, a release/install path, and the optional espflash/Rust port to drop Python from the *setup* path (runtime is already Python-free). ADR-151 §7 (A4).
- **ThroughNet Phase A / A3 (hardware gate) — in-app scan / flash / provision, verified on a real ESP32-S3.** Completes A3: the onboarding flash + connect steps now drive the board over USB. Three localhost-only sensing-server endpoints wrap our *proven* tools as subprocesses (ADR-151's "start with subprocess, port to espflash/Rust in A4"): **`GET /api/v1/setup/scan`** lists `/dev/ttyACM*`/`ttyUSB*` and probes each with `esptool flash_id` → `{ ports: [{ path, chip, flash_size, mac, ok, error }] }`; **`POST /api/v1/setup/flash {port}`** writes the bundled ThroughNet S3 firmware (`release_bins/throughnet-s3-nodisp/`, offsets `0x0/0x8000/0xf000/0x20000`, hash-verified) and reports `{ success, elapsed_s, log }`; **`POST /api/v1/setup/provision {port, ssid?, password?, target_ip?}`** runs `provision.py --auto` (fleet auto-id: first board = TX illuminator, later = RX locked to the TX MAC) and reads the final `{ role, node_id }` back from the per-port state. Honest behaviors: a `port` must be a real device node (shape guard + `exists()`; argv-not-shell, so no injection); omitted creds **merge from the board's prior state** (re-provisioning a known board needs none); `target_ip` is the mDNS *fallback* and is **never clobbered** for a board that already has stored state (auto-detected only for a brand-new board). Paths resolve from env (`THROUGHNET_SETUP_PYTHON`/`_FIRMWARE_DIR`/`_PROVISION_PY`) with repo-relative defaults. Frontend (`tn-setup.ts` + `tn-onboarding`): the flash step auto-scans → shows the detected board → flashes (with rescan/retry + honest errors); the connect step provisions and shows the assigned role/node in a fleet row; `?mock` keeps both serverless. **Verified end-to-end on a live ESP32-S3 (MAC `3c:0f:02:d7:4a:60`, 16 MB):** scan → flash → provision both via curl *and* clicked through the app UI (Vite proxy → server → board), restoring it to node 3 RX (KANAYAM) with `target_ip` preserved; with the board streaming, `/api/v1/nodes` shows `csi_fps≈137` and the doctor's `csi_ingest` check flips to **ok**. 3 new unit tests (flash_id parse, port-shape guard, state-path). ADR-151 §7 (A3 complete).
- **ThroughNet Phase A / A3 (no-hardware half) — in-app setup doctor preflight (`/api/v1/setup/doctor`).** The onboarding "prepare" step is now driven by a **real** host preflight instead of hardcoded checks. New localhost-only, read-only endpoint `GET /api/v1/setup/doctor` on the sensing-server returns `{ ok, checks: [ { id, label, status: ok|warn|fail|info, detail, fix } ] }` for the exact traps live bring-up hit, each with a copy-paste fix: **serial access** (is the user in `uucp`/`dialout` to write `/dev/ttyACM*`, plus which boards are plugged in now), **firewall** (the #1 gotcha — `ufw` silently eats inbound CSI on UDP 5005; honest about non-root: not-installed/inactive → ports open, active+readable → verify 5005/5353, active+root-only → warn with the fix rather than claim a pass), and **CSI ingest** (are frames actually arriving — ties the firewall check to reality; `info` pre-flash, `ok`/`warn` once boards stream). Linux checks `#[cfg]`-gated (info fallback elsewhere); the handler snapshots ingest state and releases the lock before any subprocess. Frontend: `app/src/onboarding/tn-setup.ts` (`fetchDoctor` + `MOCK_DOCTOR`) and a data-driven prepare step in `tn-onboarding` (loading / error+retry / `re-check` / honest `continue` vs `continue anyway`), with `fail`/`info` check styles. Verified end-to-end on this machine: the real endpoint reports serial **ok**, firewall **warn** (ufw active, unverifiable without root), CSI ingest **info** (no frames yet), and the UI renders it through the Vite proxy; `?mock` keeps it serverless. 2 unit tests; full workspace suite green. **Hardware gate (A3b):** `/api/v1/setup/{scan,flash,provision}` (board probe + `espflash` flash + `provision.py --auto`) need an ESP32 on USB and land separately. ADR-151 §7 (A3).
- **ThroughNet Phase A / A2 — embed the UI into the sensing-server (single binary).** The `app/` build (`app/dist`) is now embedded into the `sensing-server` binary via **`rust-embed`** and served at `/` (with a SPA fallback for `/assets/*`), gated behind a new **`embed-ui`** cargo feature (optional `rust-embed` + `mime_guess` deps) so the default workspace build/test needs no `app/dist`. Implementation: `/` routes to a `root_handler` (embedded `index.html` with the feature, else the existing info page); a `fallback` serves embedded assets with correct MIME types; `debug-embed` makes it a true single binary in debug too. Verified: `target/debug/sensing-server --features embed-ui` serves the app at `http://localhost:8080/` (index + `/assets/*.js` as `text/javascript`) **and** the REST API (`/api/v1/throughnet/status`, `/api/v1/nodes`) on the same origin — and it still serves with `app/dist` moved away (proving it's embedded, not read from disk). Default (non-embed) build + tests unchanged (47 sensing-server tests pass). Added `scripts/throughnet-app.sh` — builds the UI, builds the server `--release --features embed-ui`, runs it, and opens it app-mode in Chromium (`--app=`). ADR-151 §7 (A2).
- **ThroughNet Phase A / A1 — per-node link health (RSSI + packet rate) on the home + Nodes screen.** The ThroughNet verdict status (`/api/v1/throughnet/status`) carries presence/motion/breathing but not radio link health, so the node cards had no RSSI/rate. Exposed the per-node **CSI frame rate** on `/api/v1/nodes` (`csi_fps`, from the existing `NodeState::csi_fps_ema`; null until the EMA has ≥5 samples, plus `csi_fps_samples`) alongside the already-present `rssi_dbm`. The app's `TnClient` now does a best-effort second fetch of `/api/v1/nodes` each tick and merges `rssiDbm` + `csiFps` into each node by id (optional — the verdict still renders if it 404s). Surfaced as a **LINK** column in the Nodes table (`-52 dBm · 29 Hz`) and on the home node-health overlay card. Verified through the Vite proxy against a mock serving both endpoints.
- **ThroughNet Phase A / A1 — the home is live: real sensing transport + overlay cards + Nodes/Events/Settings.** Wired the diorama home to the sensing-server. `TnClient` (`src/live/tn-client.ts`) polls `GET /api/v1/throughnet/status` (~1 Hz — the authoritative ThroughNet verdict, computed server-side) against the exact JSON contract read from `sensing-server` `main.rs`, and maps it to the diorama's `SensingState`; served same-origin in production (rust-embed), Vite proxies `/api` + `/ws` → `127.0.0.1:8080` in dev. The home (diorama-as-home) shows **live overlay cards** — a breathing card (bpm + trace + confidence) and a node-health summary — with a top-right connection chip and **honest states throughout**: connecting / offline / calibrating / standby (`no_baseline_or_data`) / absent / present·still / present·moving. A slim nav (**live / nodes / events / settings**) opens three secondary screens: **Nodes** (`tn-nodes` — per-node present/moving, presence+motion scores, breathing, baseline/stale, last-seen), **Events** (`tn-events` — a session log derived from *observed* state transitions, not fabricated), **Settings** (`tn-settings` — recalibrate wired to the real `POST /api/v1/throughnet/baseline/start|stop`; Wi-Fi / add-node re-enter onboarding; about). The manual control panel stays behind `?mock` for serverless demos. Verified end-to-end against a mock server through the proxy (present·still + 15.4 bpm renders live; killing the server flips to "Sensor Offline"). Builds clean (37 modules).
- **ThroughNet Phase A / A1 — onboarding flow + signature paper-tear intro (`tn-onboarding`).** Designed and built the first-run setup UX (the IA gap ADR-151 §7.A3 only sketched). A **cover page** shows the ThroughNet wordmark in an elegant calligraphy script (**Allura**, per a user reference) on an aged-parchment ground (procedural: warm base + light centre + vignette + coarse-stain & fine-mottle fractal noise — no stock image), . The signature transition is a **draggable WebGL paper tear** (`src/onboarding/tn-tear.ts`, adapted from a drag-to-tear photo shader): the cover is baked to a `CanvasTexture` and torn by a shader — a noise-displaced white→black edge texture cuts an organic rip (strictly complementary about one boundary, so no gap/overlap), the vertex shader peels each half in 3D with front/back shading. The renderer is **transparent and draws only the sheet**, so the torn gaps reveal the DOM wizard behind with no cross-fade. **Drag down to tear** (release snaps back under threshold) or click *Begin*; a hand hint teaches the gesture. (A brief detour capped the tear amount to keep the fold cleaner at extreme drags, but the fuller **uncapped** tear read better and was kept.) It reveals a **six-step wizard** — *prepare (doctor preflight) → flash → connect → place → calibrate → done* — built from the shared chrome kit, each step grounded in the real mechanics (serial/`ufw` checks, `espflash`, `provision.py --auto` fleet auto-id with a live fleet list, the bistatic placement diagram + per-link RSSI, empty-room baseline). `tn-app` routes first-run → onboarding and `finish` → the live diorama home (`?view=live` jumps straight there). Iterated visually via standalone prototypes (`app/prototypes/onboarding.html` for the CSS-crumble first cut, then `onboarding-tear.html` for the WebGL tear) + a multi-frame screenshot harness, then ported to a `tn-onboarding` Lit component (`src/styles/onboarding.css` scoped to the element). Home model confirmed: **diorama-as-home**, data as overlay cards. Step panels are mock; the real localhost `/api/v1/setup/*` endpoints land in A3.
- **ThroughNet Phase A / A1 — app scaffold + paper-craft live view (`app/`).** Stood up the product app: **Vite + Lit 3 + TypeScript** (`app/`), the first cut of the **"Living Blueprint / Paper-craft" design system** (ADR-151-driven, the visual language chosen 2026-06-14 from Make Me Pulse 2019). Tokens (`src/styles/tokens.css`: paper/ink palette, clay/slate/sage/sand, Fraunces/Space-Grotesk/Space-Mono) + a reusable chrome kit (`src/styles/app.css`: `.tn-banner` placed-paper labels, `.tn-cap` captions, `.tn-frame` corner brackets, `.panel`/`.seg`, the auto-orbit `.viewbtn`) — the same kit onboarding/flashing/settings will reuse. The **Main Live View** is a `tn-live-view` Lit component wrapping a ported three.js diorama (`src/live/diorama.ts`, a typed `createDiorama()` handle): a paper-craft meadow on a gradient-sky horizon — a **floating faceted presence crystal** that breathes at the live BPM, **bigger floating node crystals** on a stone "room" platform, rippling RF links, grass + tree groves + gentle fog, and a stop/start auto-orbit toggle. The app shell (`tn-app`) holds the canonical `SensingState` and drives it from a **mock control panel** standing in for `GET /api/v1/throughnet/status` + the `/ws/sensing` push; clicks flow immutably through a Lit property binding into the diorama (gsap state transitions) — verified end-to-end (present·still / moving / absent) in a real Chromium render. Ported from the approved standalone prototype (`app/prototypes/live-view.html`); builds clean (`vite build`, 31 modules). Remaining A1: wire the real transport + the product cards (node health, calibration, event log, honest empty/degraded states). ROADMAP Phase 4 / Phase A, ADR-151 §7 (A1).
- **ThroughNet Phase A — app stack decided: ADR-151 server-as-shell.** The research spike that opens Phase A (our own product app). Refined the 2026-06-13 framing ("native shell needed because browsers can't do serial"): a native server *we already ship* can do serial itself and expose flash/provision over HTTP, so the browser never needs serial — collapsing the "native shell" requirement into the sensing-server binary. **Decision:** the product is **one self-contained binary** — the sensing-server serves an embedded, from-scratch Lit UI (`rust-embed`) + localhost-only `/api/v1/setup/*` endpoints (native `espflash` flash + `--auto` provision; the server owns USB, the browser renders), launched app-like (Chromium `--app=` / `.desktop`), with a doctor preflight for the known Linux traps (`uucp`/`dialout`, `ufw` 5005/5353, port conflicts). Chosen over **Tauri v2** (adds toolchain + a system WebKitGTK runtime dep that hurts Linux install, can't run headless — kept as a future *wrap* since the UI is reusable), **Electron** (bundles Chromium+Node, foreign runtime to our all-Rust codebase), and **web+thin-helper** (the "helper" is just the server we already ship). Wins: no new toolchain (CI stays `cargo build`), no webview ceiling (real browser → latest CSS/WebGL/WebGPU), headless/always-on/multi-client. Honest limit accepted: serial flashing only on the machine running the server. UI built from scratch (own design system, `dashboard/` Vite+Lit+TS+Playwright/axe *pattern* only); inherited RuView Tauri crate (ADR-052) is a protocol-fact reference, not a dependency — we drive our *own* proven tools (`provision.py --auto`, `espflash`, the server's `mdns-sd`). Milestones A1–A4 in ADR-151 §7. ROADMAP Phase 4 / Phase A.
- **ThroughNet Phase R / R3 — scripted resilience gates (`tools/gate/resilience.py`).** Turns the two hard robustness requirements into measured pass/fail. `ip-change`: records the live nodes, prompts the operator to flip the host IP, then waits up to 90 s and passes only if every live node recovers (firmware mDNS watch re-resolved + streaming resumed). `add-node`: prompts the operator to provision + power a new RX (`provision.py --auto`) and passes only if a new node id appears and streams while every existing node stays live (undisturbed). Both sample `/throughnet/status` before/after; a node is "live" if it sent a frame within ~12 s and isn't `stale`. Scoring is pure functions verified by `--selftest` (8 cases: liveness filtering, IP-change recover/never-return/no-baseline, add-node join/no-new/disturbed/dead-new), stdlib-only, exit 0/1. Documented in `tools/gate/README.md`.
- **ThroughNet Phase R / R2 — multi-node hardening (adding/removing nodes can't break the system).** Two parts. (1) **Zero-config fleet growth** in `provision.py --auto`: provisioning derives a board's identity from the rest of the fleet's per-port state instead of the operator hand-picking ids (the one real collision risk) — first board provisioned becomes the **TX illuminator**, every later board an **RX** locked to the TX's MAC, and `node_id` is the lowest free id. The TX's MAC is read from the device (`esptool read_mac`) at provision time and stored (`wifi_mac`) so RX boards can `--auto`-lock their `filter_mac` onto it. Explicit flags / prior per-port state always win. 8 unit tests. (2) **N-agnostic, staleness-aware fusion** in the server: the audit confirmed the `throughnet` map and `/throughnet/status` fusion already iterate dynamically (no fixed 2-/3-node assumption), but found a real gap — `present()`/`moving()` had **no staleness gate** and the map is never evicted, so a dropped/unplugged RX (or one reconnecting after an mDNS IP change) would stay stuck at its last verdict and the OR-fusion would pin the room to a **phantom `present` forever**. Extracted the fusion into a pure, testable `throughnet::fuse()` that **excludes nodes with no frames for `FUSION_STALE_AFTER` (15 s)**; `/throughnet/status` now also reports per-node `stale`. 5 fusion unit tests (N≥3 OR, best-link breathing, stale-node exclusion, n-agnostic scaling). 611→ sensing-server tests pass.
- **ThroughNet Phase R / R1 — mDNS server discovery (router/IP-change resilience).** Boards streamed CSI to a static NVS `target_ip`, so a host/router IP change (DHCP, new router) silently killed the stream — the exact fragility live bring-up hit. The sensing server now advertises **`_throughnet._udp.local`** (`mdns-sd`, with `enable_addr_auto` so a new lease re-announces automatically; started beside the UDP receiver for `--source esp32` only, non-fatal if 5353/udp is unavailable). The ESP32 firmware (new `mdns_resolver.c`) resolves the service after WiFi connect, **prefers the advertised A-record on its own STA subnet** (a multi-homed host advertises several), and feeds the existing `stream_sender_init_with()`; a **30 s watch task re-points the sender** if the server's IP later changes; the NVS `target_ip` stays as a **fallback** if mDNS yields nothing within the timeout, so behavior never regresses below the pre-mDNS firmware. `stream_sender` re-init is now socket-safe (closes the old fd + clears ENOMEM backoff so the watch can re-point cleanly). Server compiles clean; firmware builds against **ESP-IDF v5.4 + `espressif/mdns` 1.11.1** via a new containerized Linux build path (`espressif/idf` Docker, builds as the invoking UID into `build_docker/`). Honest boundary: mDNS fixes IP changes on a given LAN — a genuinely new SSID still needs WiFi re-provision. Live-fleet verify (IP-change survival + add-a-node) is the next step. ROADMAP Phase R.
- **ThroughNet Phase 2.4 — respiratory-rate estimator (`breathing.rs`), offline-validated + live ±2 BPM PASS (2026-06-13).** Breathing lives in the band the motion detector rejects (0.15–0.5 Hz). New `BreathingEstimator` reusing the `throughnet` `Biquad` cascade: per-bin 0.15–0.5 Hz bandpass → select the in-band-energy bins → DFT spectral peak over a ~43 s window, **searched in the band interior (0.20–0.45 Hz = 12–27 BPM)** to dodge low-edge drift/filter-corner artifacts, parabolic-interpolated for sub-bin resolution → BPM; confidence = peak prominence (peak ÷ median), reporting **"unknown"** below threshold. Uses the **measured fps** for the rate (the assumed-vs-actual footgun pegged edge HR at ~45 BPM — #987); only runs while `present_still` (buffer cleared otherwise, so an estimate uses sustained-still data only); the best-confidence link is reported. Exposed as `breathing_bpm` + `breathing_confidence` on `/api/v1/throughnet/status`. The offline study against raw captures (incl. a heavy-breathing still) drove three findings: the 0.15 Hz low edge is essential (sub-band drift otherwise dominates), bins must be picked by *in-band* energy (not amplitude), and the search must avoid the low edge (a 4th-order high-pass made it *worse* — corner ringing). Result: a still subject reads **18.5 BPM (conf 3.6)**, empty correctly reads "unknown" (conf 1.4–2.0). Known limit: a strong slow coherent sway can still leak near the low edge. 5 unit tests (incl. a measured-rate-scaling guard). **Validated live on the 3-board fleet (2026-06-13):** a paced 15 BPM run (fresh empty-room baseline, 110 s poll) read **15.0 BPM, error 0.0, conf 4.5 → PASS** — 24 post-lock `present_still` readings (range 13.8–16.7), locked to 15.0–15.7 with confidence rising to ~10 over a sustained 30 s window, 0 moving/absent misfires. The best-link picker leaned on the stronger receiver (node 2); the weaker link rarely cleared the confidence floor — a geometry note, not a detector fault.

### Fixed
- **ThroughNet OTA-over-network is now live across the entire fleet — all three nodes flashed + keyed, verified on hardware (2026-06-15).** Closes the two pending OTA caveats (the "remaining live-verify step" in *Added* and the migration note below). Both remaining boards were USB-flashed with the fixed firmware (app-only, so NVS / provisioning is preserved) and given OTA PSKs: node 2 (RX) via the server's `setup_provision` auto-PSK path; the **TX illuminator** via `provision.py` with **every field explicit** (deliberately *not* `--auto` — its port-keyed merge would have re-flashed the illuminator as an RX clone, the documented mesh-kill trap). Node 1's only OTA-PSK record lived in a stale port-keyed state file; it was preserved to a stable record *before* the port file was overwritten, and the per-port state was rationalised to **exactly one stable record per node** (no duplicate `node_id`s, the stale-port trap eliminated). Result: nodes 1 & 2 report `ota_ready`; the **TX is OTA-keyed and IP-reachable**, its `GET :8032/ota/status` reports the fixed image (`max_size` 1.5 MB — proving the size-gate fix is in the running firmware) and `POST :8032/ota` is fail-closed (HTTP 403 without the Bearer key). The RX nodes never lost lock during the rollout (the bistatic mesh stayed healthy throughout). **Honest gaps:** (1) the beacon-only TX sends no CSI, so the server can't auto-learn its IP — a one-click TX OTA needs an ARP/mDNS fallback in `setup_ota` (today the TX is updatable by supplying its IP from `ip neigh`), which is why its `ota_ready` is false; (2) the shipped `--features bundle` binary still needs a rebuild so its *embedded* firmware carries the fix. ADR-151 §7 (fleet mgmt) + ADR-050.
- **ThroughNet OTA-over-network now completes on real hardware — two firmware bugs found by live-verify (2026-06-15).** The new `POST /api/v1/setup/ota` path was verified end-to-end on a live ESP32-S3 (node 1), and the first push exposed two genuine firmware faults that would have made OTA silently fail in the field (caught exactly because we test on real hardware, not mock): (1) **`httpd` task stack overflow** — the OTA upload handler holds a 1 KB receive buffer on the httpd task's default 4 KB stack and then calls `esp_ota_write()` (deep flash-driver path); the task overflowed and panic-rebooted at **~93%** of a 900 KB image, *before* the A/B commit, so the partition never flipped and the host saw a dropped connection. Fixed by `config.stack_size = 8192` in `ota_update.c`. (2) **`OTA_MAX_SIZE` too small** — the firmware capped accepted images at 900 KB (an arbitrary CI gate), but the real image is **944 KB** (grew after an ESP-IDF component bump); the node 400'd and reset the connection before reading the body (host saw `ECONNRESET`). Raised to 1.5 MB (the `ota_0`/`ota_1` partitions are 2 MB each). With both fixes flashed, the OTA push **succeeded in 9 s** and `/ota/status` confirmed the running partition flipped **`ota_0` → `ota_1`** with node 1 rejoining the mesh. Refreshed `release_bins/throughnet-s3-nodisp/` (the on-disk image the server streams) + `SHA256SUMS.txt`. **Migration (completed 2026-06-15):** nodes 2 & 3 were USB-flashed with this build and PSK-provisioned (see the fleet-wide rollout entry above) — the whole fleet now runs the fixed firmware. The shipped bundle binary still needs a rebuild (`--features bundle`) so its *embedded* firmware also carries the fix.
- **ThroughNet fleet view no longer leaks phantom "unprovisioned" rows from stale node-state — fleet-mgmt follow-up (2026-06-15).** Stray/malformed UDP on the ingest port accrues bogus node-ids in the in-memory `node_states` map, and they aren't evicted while the ingest loop sits idle (no frames → no prune tick) — so after a long idle session the Devices console / reconciler surfaced ~50 phantom "new board" rows (`setup_fleet` §4c emits every uncovered live node, regardless of freshness). The §4c join now only surfaces uncovered nodes that are **actually streaming** (`online`); an uncovered *offline* node is a stale artifact, not an actionable board. (A server restart also clears them, since `node_states` is in-memory — observed dropping 54 → 0.) Mirrors the 60 s eviction the parallel `throughnet` detector map already got (2026-06-14). The deeper fix — evicting idle `node_states` on a timer rather than per-frame — is noted for follow-up.
- **ThroughNet TX illuminator no longer reads "offline" in the fleet view while the mesh is healthy — fleet-mgmt follow-up (2026-06-15).** A bistatic TX beacons but, once RX peers join the mesh, sends no CSI of its own — so it never shows `online`, and the Devices console / onboarding fleet rows rendered it as **offline** even with the mesh up (the same root cause as the earlier `summary.healthy` fix, one layer down: per-device status). The reconciler (`GET /api/v1/setup/fleet`) now derives, per device, a **`link_up`** field and (for a known TX inferred up via a live RX) a new **`illuminating`** bucket: a radio-silent RX only streams while it's capturing the TX beacon, so ≥1 live RX *implies* a live illuminator. `online` stays the literal "fresh CSI frames" truth; `link_up` is the "contributing to a working link" signal. The UI (`tn-devices`, `tn-onboarding`) keys its status pill / link text / gem on `link_up` and shows the TX as **"beaconing"** instead of "offline". Two pure helpers (`device_link_up`, `device_bucket`) factor out the inference; 1 new unit test (`beacon_only_tx_reads_up_when_an_rx_streams`) → 152 sensing-server tests pass; app builds; rendering verified in `?mock` (TX node 1 reads "beaconing", RX nodes stream). Side benefit: restarting clears the stale in-memory `node_states` that had been leaking ~50 phantom "unprovisioned" rows into the fleet list. ADR-151 §7 (fleet mgmt).
- **ThroughNet `throughnet` detector map is now evicted on the same 60 s policy as `node_states` — Phase R follow-up (2026-06-14).** The per-node `node_states` map is pruned every 100 ticks for nodes silent ≥60 s, but the parallel `throughnet` `LinkDetector` map was **never evicted**, so over a long run it accumulated bogus node ids from occasional malformed/stray UDP — they lingered in `/api/v1/throughnet/status` and cluttered the app's node list (a fresh server is clean; only nodes 2/3). The R2 staleness gate (`FUSION_STALE_AFTER` = 15 s) already kept these out of *fusion*, but not out of the status surface or memory. Mirrored the existing eviction onto `s.throughnet`, keyed on **last frame arrival** (new `LinkDetector::last_frame_time()`, the last `recent_frames` timestamp) rather than `last_update` — `last_update` is only set on a scored window and stays `None` for a node streaming pre-baseline, so using it would mis-evict a live node. A new unit test (`last_frame_time_tracks_arrival_without_baseline`) pins that distinction (frame time registers pre-baseline while `last_update` is still `None`; a malformed too-short frame leaves the timer unchanged); 13 throughnet tests pass.
- **ESP32 raw-CSI parser read `rssi`/`noise_floor`/`sequence` from the wrong byte offsets — `parse_esp32_frame` (2026-06-13).** The server's header decode had drifted from the firmware's `csi_serialize_frame` layout (`csi_collector.c`): it read `n_subcarriers` as a u8 at `[6]` (firmware writes u16 at `[6..7]`), `freq_mhz` as u16 at `[8..9]` (u32 at `[8..11]`), `sequence` at `[10..13]` (should be `[12..15]`), and crucially **`rssi` at `[14]` and `noise_floor` at `[15]` instead of `[16]`/`[17]`**. Magic, `node_id`, and length checks still passed and the I/Q payload (offset 20) was correct — so ThroughNet, which uses amplitudes, was unaffected — but **every frame's RSSI decoded to 0** (confirmed live: `/api/v1/sensing/latest` `mean_rssi: 0.0` while boards reported −43…−62 dBm) and sequence numbers were garbage, corrupting per-node RSSI history and any RSSI-derived logic. Realigned all five fields to the firmware layout and widened `Esp32Frame.n_subcarriers` to u16 / `freq_mhz` to u32. New regression test (`parses_firmware_header_at_correct_offsets`) builds a frame from the real on-wire byte layout and asserts every field. Found during the Phase 2.4 live breathing bring-up; sensing-server tests pass.
- **ThroughNet Phase 2 COMPLETE — motion still-vs-walking now 100% on hardware via motion-band energy (2026-06-13).** The relative-motion metric (below) plateaued ~85% and a first motion-band attempt (a single broad 0.5–5 Hz bandpass) *leaked breathing*: a subject standing still on a link line, breathing normally, read motion 4–9× and was misclassified as walking (live run). Root cause: the broad bandpass (Q 0.35) had a low skirt reaching ~0.25 Hz, passing respiration. Fix: replaced it with a **cascaded 2nd-order Butterworth high-pass (1 Hz) + low-pass (6 Hz)** — steep enough to reject breathing, its harmonics, and slow postural sway while keeping walking's sustained Doppler — with the windowed band power normalized by an empty-room floor (`motion_band_floor`, captured at baseline) and fused (OR) across links. Tuned offline against raw captures including a deliberately heavy-breathing still (`raw_capture.py`), then confirmed live: fused still ≤ 2.5× the floor (breathing on n3's line dropped to 0.6–0.8×), walking ≥ 3.5× → threshold **3.0×** gives **100%** (93% at the initial 2.5×). All four Phase-2 acceptance gates now pass on real hardware: presence FP **0%**, detection **100%**, latency **0 s**, motion **100%** — including the hardest case (still subject breathing on a link line). New `Biquad`/`BiquadState` (HP/LP) + `is_moving`; `motion_score` is now the normalized band power; `MOTION_BAND_THRESH`. Real-data unit tests; 606 sensing-server tests pass.
- **ThroughNet motion no longer reads a still subject on a strong link as "moving" — relative-motion metric (Phase-2 hardware validation, 2026-06-13).** First live run of `validate.py` on the 3-board fleet passed all three presence gates (false-positive **0%**, detection **100%**, latency **0 s**) but failed motion still-vs-moving at **50%**: standing still *on node 3's link line* produced a huge but *stable* perturbation (presence 196×), and the absolute motion metric (window-to-window profile change ÷ empty-rate) scaled with that perturbation, so breathing/sway there read motion 50× — *higher* than walking's 6×, so no threshold could separate them. Replaced the motion observable with **relative motion** (`motion ÷ presence`, the fractional change, gated on presence): still ≈ 0.19–0.30 vs walking ≈ 0.46–1.21 (median). Run 2 confirmed the fix — standing still on n3's line now reads `present_still` **44/45** (was `present_moving` 39/44). New `is_moving_rel` helper + a real-data regression test (`relative_motion_separates_still_from_walking`); `motion_rel` exposed on `/api/v1/throughnet/status`. **Known limitation:** still/walking discrimination only reaches ~80–88% (below the 90% gate) and is run-dependent, because walking's *relative* motion dips when the subject pauses; the robust fix (0.5–5 Hz motion-band spectral energy) is the next Phase-2 item. Presence is production-ready.
- **ThroughNet presence/motion state no longer flips on a single transient window — Phase-2 polish.** The Phase-2 live validation (`tools/gate/README.md`) showed the detector's raw presence/motion scores were correct but the fused state (`/api/v1/throughnet/status`) blipped to `present_moving` on lone transient windows — a walk-in transient and a weight-shift on the hot −52 dBm link during the "still" protocol. `present()`/`moving()` in `throughnet.rs` were bare threshold checks (`score >= 3.0`) with no hysteresis, and the fusion handler ORs them statelessly. Added `commit_state` debounce at the **detector** level (advances on window emission, not poll rate): the first scored window commits immediately (unknown→known), but a subsequent flip requires `DEBOUNCE_WINDOWS` (=2) consecutive disagreeing windows — ~4 s at the ~2 s window cadence, inside the <10 s latency budget — so an isolated window can't change state. Raw `presence_score`/`motion_score` stay exposed on the status API for diagnostics; the OR fusion in `main.rs` is unchanged (it now ORs debounced booleans). 5 throughnet tests (incl. immediate-first-commit, single-transient-ignored, two-consecutive-flip); 604 sensing-server tests pass.
...
### Added
- **ThroughNet Phase-2 validation harness — `tools/gate/validate.py`.** The piece that makes the detector *measurable* instead of eyeballed (ROADMAP §4, Phase-2 item 5): drives the running sensing-server's ThroughNet REST API (`/throughnet/baseline/start|stop`, `/throughnet/status`) through scripted ground-truth phases (baseline → empty → enter+still → moving → exit), records the fused state + per-node scores each poll, and scores the run against the four Phase-2 acceptance gates — presence false-positive <5% (empty), presence detection >95% (occupied), presence latency <10 s (enter→detect), motion still-vs-moving discrimination >90% (5 s windows). Reuses only existing endpoints (no server changes). Scoring is pure functions verified by `--selftest` (clean run passes all gates; injected faults — 14 s latency, 40 empty-room blips, inverted motion, empty input — each trip the correct gate). A 10 s post-phase settle allowance keeps the debounce/posture transient out of steady-state metrics while latency is measured on the transient. Stdlib-only; `--quick`, `--check`, `--out <json>` modes; exits 0/1 on PASS/FAIL.
